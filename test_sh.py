...
from data.data_loader_sh import Dataset_SH_minute
from torch.utils.data import Dataset, DataLoader
from model.net import stgt
import torch
import pandas as pd
import numpy as np
...
parser.add_argument('--root_path', type=str, default='data/SHMetro/', help='data root path')
parser.add_argument('--data', type=str, default='SHMetro.csv', help='data path')

parser.add_argument('--adj_data', type=str, default='data/SHMetro/graph_sh_conn.pkl', help='adj data path')
parser.add_argument('--dtw_data', type=str, default='data/SHMetro/graph_sh_cor.pkl', help='adj data path')
parser.add_argument('--st', type=str_to_bool, default=True, help='whether to add spatial transformer')
...
parser.add_argument('--num_nodes', type=int, default=288, help='number of nodes/variables')
...
parser.add_argument('--out_dim', type=int, default=2, help='outputs dimension')

parser.add_argument('--seq_in_len', type=int, default=4, help='input sequence length')
parser.add_argument('--label_len', type=int, default=2, help='label length of decoder')
...
parser.add_argument('--clip', type=int, default=5, help='clip')  # 5
...
# parser.add_argument('--save', type=str, default='./save/sh_log', help='save path')
parser.add_argument('--save', type=str, default='./save/sh_919', help='save path')
...
model = stgt(gcn_true=args.gcn_true, st=args.st, gcn_depth=args.gcn_depth, num_nodes=args.num_nodes,
             device=args.device,
...
# path = args.save + "exp" + str(args.expid) + "_" + str(0) + str(args.layers) + '_' + str(
#                            args.epochs) + '_' + str(
#                            args.d_model) + str(args.label_len) + '_' + str(args.dropout) + '_' + str(
#                            args.learning_rate) + ".pth"
path = args.save + "exp" + str(args.expid) + "_" + str(0) + str(args.layers) + '_' + str(
    args.epochs) + '_' + str(
    args.d_model) + str(args.label_len) + '_' + str(args.dropout) + '_' + str(
    args.learning_rate) + ".pth"
...
engine.model.load_state_dict(torch.load(path, map_location=torch.device('cpu')))
